...
backup_to_s3_container() {
    ...
    [ ! -z "${GPG_KEY}" ] && gpg_options="--encrypt-key=${GPG_KEY} --sign-key=${GPG_KEY}" &&
        [ -z ${PASSPHRASE+x} ] && exit_fatal_message "PASSPHRASE must be defined"
...
delete_older_backup_from_s3_container() {
    local timstamp=$1
    ...
    duplicity remove-older-than ${timstamp} ${duplicity_options} ${gpg_options} ${duplicity_target}
